...
const MAX_TOKENS = 1048576; // Based on Gemini 1.5 Flash capacity.
...
interface ImprovActions {
    ...
    sendDirectorNote: (note: string) => Promise<void>;
...
let aiForTokenCount: GoogleGenAI | null = null;

const getAiForTokenCount = (): GoogleGenAI | null => {
    if (aiForTokenCount) return aiForTokenCount;
    const apiKey = process.env.API_KEY as string;
    if (apiKey) {
        aiForTokenCount = new GoogleGenAI({ apiKey });
        return aiForTokenCount;
    }
    console.warn("API key not found for token counting.");
    return null;
}
...
export const useGeminiImprovSession = create<ImprovState & ImprovActions>((set, get) => ({
  ...
  addTokens: (amount: number) => {
    set(state => ({
        currentTokenCount: state.currentTokenCount + amount,
        totalTokenCount: state.totalTokenCount + amount,
    }));
  },

  reset: () => {
    // Keep totalTokenCount across full resets
    set({ ...initialState, totalTokenCount: get().totalTokenCount });
  },
  ...
  startSession: async (config) => {
    ...
    set({ status: 'connecting', error: null, currentActorTranscript: '', currentAiTranscript: '' });
...
### 6. NOTAS DEL DIRECTOR
-   Ocasionalmente, recibirás instrucciones en tiempo real entre corchetes, como "[NOTA DEL DIRECTOR: Revela un secreto]".
-   Estas no son parte del diálogo. Son instrucciones para ti como actor.
-   Tu tarea es incorporar sutilmente la esencia de esa nota en tu siguiente diálogo, de una manera que se sienta natural y dentro del personaje. NUNCA menciones la nota explícitamente.
...
`;
      ...
      const tokenCounter = getAiForTokenCount();
      if (tokenCounter) {
          try {
              const { totalTokens } = await tokenCounter.models.countTokens({ model: MODEL_NAME, contents: initialContextMessage });
              set(state => ({
                  currentTokenCount: totalTokens,
                  totalTokenCount: state.totalTokenCount + totalTokens
              }));
          } catch (e) {
              console.error("Failed to count initial tokens:", e);
          }
      }
                         ...
                         if (tokenCounter) {
                             (async () => {
                                 try {
                                     let newTokens = 0;
                                     if (actorText) {
                                         const { totalTokens } = await tokenCounter.models.countTokens({ model: MODEL_NAME, contents: actorText });
                                         newTokens += totalTokens;
                                     }
                                     if (aiText) {
                                         const { totalTokens } = await tokenCounter.models.countTokens({ model: MODEL_NAME, contents: aiText });
                                         newTokens += totalTokens;
                                     }
                                     if (newTokens > 0) {
                                         get().addTokens(newTokens);
                                     }
                                 } catch(e) {
                                     console.error("Failed to count turn tokens:", e);
                                 }
                             })();
                         }
                      }
...
**OBJETIVO:** Mantener una memoria de la historia que sea concisa, precisa y fácil de consultar para la IA en la siguiente escena. El formato de salida DEBE ser un párrafo narrativo.
...
**4. SALIDA REQUERIDA:**
Basado en tu análisis, genera un **párrafo narrativo, conciso y actualizado** que resuma los Hechos Clave. Responde ÚNICAMENTE con el párrafo. NO incluyas tus notas de proceso mental ni ningún otro texto introductorio. NO uses viñetas ni saltos de línea.

Ejemplo de formato de salida:
${aiName} y ${actorName} ahora están casados. Sofía es una amiga de la pareja que trabaja como organizadora de bodas, y ${aiName} cree que Gastón, un ex compañero del colegio, le debe dinero.
`;
  ...
  sendTextMessage: async (text) => {
    ...
    if (tokenCounter) {
        (async () => {
            try {
                const { totalTokens } = await tokenCounter.models.countTokens({ model: MODEL_NAME, contents: text });
                get().addTokens(totalTokens);
            } catch (e) {
                console.error("Failed to count text message tokens:", e);
            }
        })();
    }
  ...
  sendDirectorNote: async (note) => {
      const { status } = get();
      if (status !== 'listening' && status !== 'speaking') return;

      const formattedNote = `[NOTA DEL DIRECTOR: ${note}. Incorpora sutilmente esta idea en tu próxima respuesta, sin mencionar la nota directamente.]`;
      
      try {
          if (!sessionPromise) return;
          const session = await sessionPromise;
          session.sendRealtimeInput({ text: formattedNote });
          set({ status: 'speaking' });
      } catch (err: any) {
          console.error("Failed to send director note:", err);
          set({ status: 'error', error: `Falló al enviar la nota: ${err.message}` });
      }
  }
}));
